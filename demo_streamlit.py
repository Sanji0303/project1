...
with tab2:
    ...
    **Hướng dẫn:**
    ...
    def preprocess_batch_data(df):
        ...
        # Mapping giá trị cho các cột phân loại
        ...
        # Tạo dataframe mới với các cột chuẩn
        standardized_df = pd.DataFrame()
        warnings = []
        
        # Xử lý từng cột bắt buộc
        ...
        try:
            ...
            if warnings:
                with st.expander("⚠️ Cảnh báo xử lý dữ liệu", expanded=True):
                    for warning in warnings[:5]:  # Chỉ hiển thị 5 cảnh báo đầu
                        st.warning(warning)
                    if len(warnings) > 5:
            ...
            with st.spinner("🔄 Đang định giá cho từng bất động sản..."):
                ...
                for idx, row in df_processed.iterrows():
                    try:
                        ...
                        results.append({
                            "STT": idx + 1,
                            **{col: row.get(col, "N/A") for col in df_processed.columns[:8]},
